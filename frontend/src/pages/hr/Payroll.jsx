...
const generatePayslipPDF = (employee) => {
  ...
  try {
    ...
    const totalEarnings = earnings.reduce((sum, earning) => {
      return sum + (values[earning.key] || 0);
    }, 0);
    
    const totalDeductions = deductions.reduce((sum, deduction) => {
      return sum + (values[deduction.key] || 0);
    }, 0);
    ...
    earnings.forEach(earning => {
      earningsData[earning.key] = {
        label: earning.label,
        value: values[earning.key] || 0
      };
    });
    
    deductions.forEach(deduction => {
      deductionsData[deduction.key] = {
        label: deduction.label,
        value: values[deduction.key] || 0
      };
    });
            ...
            columns={[
              {
                ...
                render: (_, record) => (
                  <div>
                    <div style={{ fontWeight: '600', color: '#1e293b' }}>{record.employee_name}</div>
